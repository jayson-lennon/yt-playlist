...
mod tests {
    ...
    fn cancel_clears_state() {
        ...
        assert!(!rename.is_active());
        assert!(rename.input.is_empty());
        assert!(rename.take_submitted().is_none());
    }

    #[test]
    fn submit_stores_alias_when_not_empty() {
        // Given an active rename with input.
        let mut rename = Rename::new();
        rename.start(None);
        rename.push_char('n');
        ...
        // Then alias is stored and state is cleared.
        assert_eq!(rename.take_submitted(), Some(Some("new".to_string())));
        assert!(!rename.is_active());
        assert!(rename.input.is_empty());
    }
    ...
    fn submit_stores_none_when_empty() {
        ...
        assert_eq!(rename.take_submitted(), Some(None));
        assert!(!rename.is_active());
    }
    ...
    fn take_submitted_returns_none_when_not_submitted() {
        ...
        let result = rename.take_submitted();
    ...
    fn take_submitted_clears_the_stored_value() {
        // Given a rename component with a submitted value.
        let mut rename = Rename::new();
        rename.start(None);
        rename.push_char('t');
        rename.submit();
        ...
        let first = rename.take_submitted();
        let second = rename.take_submitted();

        // Then first returns the value and second returns None.
        assert_eq!(first, Some(Some("t".to_string())));
    ...
    fn handle_key_enter_submits_rename() {
        ...
        // Then rename is submitted and event consumed.
        assert_eq!(result, EventResult::Consumed);
        assert!(!rename.is_active());
    }
    ...
    fn handle_key_consumes_all_keys_when_active() {
        ...
        assert_eq!(tab_result, EventResult::Consumed);
        assert_eq!(up_result, EventResult::Consumed);
        assert_eq!(f1_result, EventResult::Consumed);
    }
}
